...
1. 연회비가 문자열로 들어와 범위를 지정해 검색하기 어려움. 숫자로 형변환 진행하여 문제 해결
2. 검색에서 띄어쓰기에 따라 다른 결과가 나옴. 정확히 입력해야지만 원하는 결과가 나옴. ( 카드의 정석 != 카드의정석 ) 따라서 띄어쓰기와 상관없이 동일한 결과가 나오도록 정규화 이용
...
1. 카드를 비교해 csv파일로 저장할 때, 행 순서를 바꾸고 싶음 → 데이터 프레임 순서를 변경하면 가능할 것으로 예상
2. Kibana를 이용해서 시각화를 통해 크롤링한 데이터의 정보만 확인하고 페이지 자체에서 보이게 구성하지는 못함. 확인한 내용과 Streamlit을 연동해서 함께 실시간으로 시각화가 뜨게 만들어보고싶음.
