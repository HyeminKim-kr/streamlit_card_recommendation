...
st.title("카드를 추천해드립니다~~")
index_name = "card_info"
field_name = "card_name"

st.sidebar.header("조회하고 싶은 카드를 입력하세요")
match_name = st.sidebar.text_input('조회하려는 내용', value="").strip()
...
def fetch_data(index_name, field_name, match_name):
    if match_name == "":
        result = search_index(index_name, field_name, "*")  # Using "*" to get all records
    else:
...
if not df.empty:
    # Get keyword for card name filtering
    keyword = st.sidebar.text_input('카드 이름 필터링 키워드', value="").strip()
    if keyword:
        # Apply filter based on keyword
        df['filtered'] = df['card_name'].apply(lambda name: contains_keyword(name, keyword))
        df = df[df['filtered']]
    
    # Extract categories for the sidebar
    categories = extract_categories(df)
    selected_classes = st.sidebar.multiselect('카테고리 클래스 선택', options=['All'] + categories, default=['All'])
